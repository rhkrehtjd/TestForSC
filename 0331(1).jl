...
md"""
`-` $F^{-1}(U)\overset{d}{=} X$   (where $X \sim F$ and $U\sim U(0,1))$
  - 여기서 F는 임의의 분포의 cdf를 의미함
  - 여기선 지수분포의 cdf를 의미하는 것
  - 따라서 아래 코드에서 
  - rand(Exponential(1),1000000) .|> x -> 1-exp(-x) 
"""
...
`-` 모든 $c$에 대하여 $P(F(X)\leq c) = P(U \leq c )$ 이 성립함을 보이면 된다.
  - 두 확률변수 X,Y가 있을 때, 각각의 cdf가 같으면 X와 Y의 분포가 같다고 볼 수 있음
    - 위의 확률표현이 cdf를 의미하는 것
"""

# ╔═╡ 504d2a53-a109-4219-b8f7-c8763e949afe
md"""
`-` 아래와 같이 계산가능 (LHS에서 두번째 등호가 성립하는 이유는 $F$가 증가함수이기 때문)
-  $RHS=P(U\leq c)=\int_0^c pdf (of) U(x)dx=\int_0^c 1 dx=c$
...
- cdf의 정의에 의하여 임의의 $\star$에서 $P(X\leq \star)=F(\star)$ 이 성립. 
-  $\star=F^{-1}(c)$ 를 대입하면 $P(X \leq F^{-1}(c))=F(F^{-1}(c))=c$ 
"""
...
(rand(Exponential(1),1000000) .|> x -> 1-exp(-x)) |> histogram
# 여기서 x -> 1-exp(-x)는 지수분포의 cdf이며 F(X)를 의미한다. 
...
# ╔═╡ b47a5de2-90dc-41bd-b140-61282954be4a
md"""
---
"""
...
md"""
- 두 확률변수 $X$, $Y$가 있을때, 각각의 cdf가 같으면 $X$와 $Y$의 분포가 같다고 볼 수 있음.
  - 위에서  $F(X)\overset{d}{=}U$ 보일 때 사용한 방법과 유사하다
...
# ╔═╡ 270ac0cb-b37a-4c9e-b669-e08df322a62e
md"""
- 즉 양변의 cdf가 동일하므로 두 분포는 동일하다. 
"""
...
begin
	...
	X=randn(N).+ 1.2 
	Y=randn(N).+ 3.4 
	# 1.2와 3.4를 중심으로 산발적인 점 100개 뿌린다. 
	scatter(X,Y,xlim=(-12,12),ylim=(-12,12),color="white",alpha=0.2)
	scatter!([1.2],[3.4],color="red") # 목표 점, list에 넣어준다!
end 
...
begin
	scatter(X,Y,xlim=(-10,10),ylim=(-7,7),color="white",alpha=0.2)
	scatter!([1.2],[3.4],color="red") # 목표점
	
	θ = 0:0.01:2π
	plot!( 1.2  .+ r.*cos.(θ) , 3.4 .+ r.*sin.(θ), color="blue")
...
begin
	if side=="앞면"
		...
		scatter!([X[i]],[Y[i]],color="blue")
		# 쏜 점 기준으로 원 그리기
		plot!(X[i] .+ r.*cos.(θ),  Y[i] .+ r.*sin.(θ)  , color="red")
		
	else 
		scatter([X[i]],[Y[i]],color="blue",xlim=(-12,12),ylim=(-12,12))
		# 쏜 점을 기준으로 원을 그려주는 것이다.
