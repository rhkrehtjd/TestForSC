...
# ╔═╡ d4ee8c20-1b3d-43c9-99ed-27db82908dc3
rand(Exponential(3),10)

# ╔═╡ 6ea24d60-1ddc-4462-af52-5009136051f8
md"""
(방법2) 포아송프로세스를 이용
- try\_until\_you_succeed 이용
"""
...
let
	p1 = plot(x -> 1-exp(-x),0,20, title="mean=1,cdf") 
	p2 = plot(x -> 1-exp(-x/5),0,20, title="mean=5,cdf")
	
	plot(p1, p2, layout=(1,2))
	# 다중 plot 그려줄 땐 인자를 comma로 넣어준다. 
end

# ╔═╡ c023b71b-a860-45b0-ab4e-059674fc24d2
md"""
- 누적분포함수 : 주어진 확률변수가 특정 값보다 작거나 같은 확률을 나타내는 함수이다. 
"""
...
let
	...
	u = rand(5) # 5개의 샘플을 유니폼에서 추출, 이제 이 값이 y값이 될 것. 
	
	p1 = plot(x -> 1-exp(-x),0,20, title="평균=1인 지수분포 cdf")
	scatter!([0,0,0,0,0],u) # 이제 이 값들에서 출발하여 x에 떨어뜨릴 것임, 주황점 
	scatter!(Finv.(u),[0,0,0,0,0]) # 떨어진 x의 값,초록점. 이 점들을 모으면 지수분포
	
	p2 = plot(x -> -exp(-x/5)+1 ,0,20, title="평균=5인 지수분포 cdf")
	scatter!([0,0,0,0,0],u)
...
let
	p1= histogram(rand(10000) .|> x -> -log(1-x),title="평균 1인 지수분포 cdf역함수")
	# 평균이 1인 지수분포의 cdf의 역함수
	# 아래로 떨어진 x값들을 모아서 histogram을 그린 것이다. 
	p2= histogram(rand(Exponential(1),10000),title="평균 1인 지수분포 pdf")
	# 평균이 1인 지수분포의 histogram 
	plot(p1,p2,layout=(1,2))  
end

# ╔═╡ e6467d7c-0643-4d1d-91b5-b0e9f8d1fb87
let
	p1= histogram(rand(10000) .|> x -> -5log(1-x))
	# 평균이 5인 지수분포의 cdf의 역함수 
	p2= histogram(rand(Exponential(5),10000))
	# 평균이 5인 지수분포의 histogram 
	plot(p1,p2,layout=(1,2))  
end
...
- 이해를 위해서 $t=1,s=9$ 대입 => $P(X>1)=P(X>10 | X>9)$
  - 지수분포 ? 특정한 이벤트의 발생 후, 바로 다음에 같은 이벤트가 발생할 때까지 걸리는 시간에 대한 분포 
  - 즉, 어떤 사건이 발생할 확률을 의미하는 P를 따질 때, 확률변수 X가  범위 1 이상이라면 시간을 1 기다려서 이벤트가 발생하지 않을 확률을 의미
- 좌변: 시간을 1 기다려서 이벤트가 발생안할 확률 
- 우변: 시간을 9 기다렸는데 이벤트가 발생 안 할 확률 -> 시간 10 기다려서 이벤트가 발생 안 할 확률 
- 예를들어서 $\lambda=0.1$ 이라면 한번 이벤트 발생하는데 평균 시간10이 걸린다는 의미임. => (1) 좌변은 이제 시간1기다림 (2) 우변은 시간 9를 기다림. 곧 "약속된" 시간 10이 완성됨 => 우변이 더 확률이 크지 않을까? => 아님. 전에 기다린 시간 9는 기억하지 못함. 무의미. 시간 9기다리고 1기다린다음에 10기다린 결과나 , 그냥 1기다린 결과나 확률은 동일하다는 것을 의미함.
- 시간9 기다렸다가 좀 더 기다려서 시간10기다린다고 시간 1기다린 것보다 확률이 더 크다는 것을 의미하지 못함.
- 시간100을 기다렸건 2000을 기다렸건 무기억성에서는 중요하지 않다. 
"""
...
let 
	...
	md"""
	-  $P(X>1)$= $(prob1)
	  - 이 확률이나 아래 네가지 확률이나 피차 다를 게 없다는 것을 의미한다.
	-  $P(X>2|X>1)$= $(prob2) 
	-  $P(X>3|X>2)$= $(prob3)
	-  $P(X>4|X>3)$= $(prob4)
	-  $P(X>5|X>4)$= $(prob5)
...
- E(X) , X ~ EXP(1)를 구하는 것과 동일
  - 포아송 분포의 평균과 분산 : λ
  - 지수 분포의 평균과 분산 : λ,   λ²
  - 베르누이 분포의 평균과 분산 : p, p(1-p)
  - 이항 분포의 평균과 분산 : np, np(1-p)
"""
...
md"""
- 단, $I(x \in A )= \begin{cases} 1 & x\in [0,1] \\ 0 & o.w \end{cases}$
"""
...
md"""
구하는것은 $E(I(X\leq 1))=P(X\leq 1),~ X\sim Exp(1)$ 로 해석 가능
"""
...
- 이때 U(0,1)가 의미하는 것은 해당 구간(0,1)에서 1값만 나오는 것
  - 만약 적분구간이 0~2였다면 균일분포의 pdf는 $\frac{1}{2}$였을 것이다.
- 즉 상수함수랑 유사한데 구간이 정해져있는 분포를 유니폼분포, 균일분포라고 한다.
  - 이 구간에 따라 pdf가 결정될 것이다. 
- 예를 들어보자
  - 평균이 1인 지수분포에 대해서 얘기해보자
  - 단지 확률변수 X에 대해서 얘기할 때 이것의 평균은 E(X)로 표현할 수 있을 것이다. 
  - 만약 확률변수 X²-1에 대해서 얘기할 때 이것의 평균은 E(X²-1)로 표현할 수 있을 것이다. 
  - 만약 확률변수 X²에 대해서 얘기할 때 이것의 평균은 E(X²)으로 표현할 수 있을 것이다. 
  - 이때 위의 세가지 경우 E(X)와 E(X²-1)와 E(X²) 모두 **확률변수 X는 지수분포를 따르는 것이다**.
- 즉 이 셀의 경우도 동일하다.
  - 균일분포, 즉 유니폼분포에 대한 설명부터 해보자
  - 균일분포는 주어진 구간에서 동일한 함수값만을 가지는 것을 의미한다. 
  - 따라서 주어진 적분 구간에서 해당 유니폼은 1만 나오는 것이다.
  - 따라서 주어진 적분 구간에서 유니폼의 pdf는 1이다. 구간에 따라서 유니폼의 pdf는 다른 상수가 될 수도 있는 것이다. 
  - 균일분포 = Uniform이 0에서부터 1까지 뽑히는데 그때의 pdf는 1이다.
  - 즉, 균일분포, Uniform의 pdf가 1이므로 해당 식에서 1이 곱해져있는 것은 위에서 설명한 네가지의 경우와 동일하다. 
"""

# ╔═╡ d2131950-e408-45a7-80a0-002d17d505ce
let 
	N = 10000
	X = rand(N) # (0,1)균등분포에서 x값들을 뽑는 것, 이 x값들을 지수분포에 넣어줄 것이다. 
	exp.(-X) |> mean
end 
...
rand(10000)*π 
# pdf가 1/π인 유니폼분포이다. 
# 여기서 적분 구간이 0부터 π이기 때문에 균일분포의 pdf를 1/π로 해주려고
# sin(x)뒤에다가 π를 곱해준 것이다.
# sin(x)뒤에다가 1/π²을 곱해주려면 적분구간이 0부터 π²이어야 할 것이다. 
...
# ╔═╡ 79a63894-b927-4d4a-9c17-f32f013c5401
md"""
----
"""
...
md"""
`-` 이론: $\begin{cases} r^2/2 \sim Exp(1) \\ \theta \sim U(0,2\pi) \end{cases} \Rightarrow \begin{bmatrix} r\cos \theta \\ r \sin \theta \end{bmatrix} \sim N\left (\begin{bmatrix} 0 \\ 0 \end{bmatrix}, \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}\right)$ 
"""

# ╔═╡ 42ec1e91-1152-47b2-a43a-55a4f7b19743
md"""
- 의미 : $\frac{r²}{2}$ 이 평균이 1인 지수분포를 따르고, θ가 pdf가 pdf $\frac{1}{2π}$인 균일분포를 따른다면 $rcosθ$와 $rsinθ$는 각각 독립인 표준정규분포를 따른다
"""
...
let 
	Random.seed!(43052)
	X=randn(5000) # 그냥 rand(균등분포)와는 달리 표준정규분포에서 뽑는 것을 의미한다. 
	Y=randn(5000) # 그냥 rand(균등분포)와는 달리 표준정규분포에서 뽑는 것을 의미한다. 
	plot(X,Y,seriestype=:scatter,alpha=0.1)

	# 여기서 X와 Y가 위 박스뮬러에서 rcosθ와 rsinθ를 의미하는 것
	# 즉 일단 X만 본다면 rcosθ가 표준정규분포를 따른다는 거고
	# 뿌려진 점들이 rcosθ 값들이므로 삼각함수 이용한 좌표라고 생각해도 됨
	# 부연하자면 만약 저 위의 수많은 점 중 (1,2)을 생각하면
	# 이때의 1은 rcosθ로 생각할 수 있다
	# 이때 r을 제곱하고 나누기 2를 하면 그게 평균이 1인 지수분포를 따른다는 것임   
	
	# 아래의 plot은 점(0,0)과 점(X[i],Y[i]) 두 점을 연결한 것이다. 
	plot!(   [ 0 , X[i] ]  ,   [ 0 , Y[i]  ]   )
	# 이게 R
	# 이걸 제곱하고 2로 나눈 것들을 모으면 평균이 1인 지수분포를 따를 것이다. 
end

# ╔═╡ 26512f21-7de0-4e50-9a76-bf84e64492a4
let 
	N=5000
	X = rand(Normal(0,1),N) # rcosθ
	Y = rand(Normal(0,1),N) # rsinθ
	p1= (X.^2 + Y.^2)./2 |> histogram # 원점과 원 위의 점 사이의 거리
	# 아까 극좌표를 이용한 직교좌표가 표준정규분포를 따른다고 했고
	# 직교좌표는 rcosθ,rsinθ임
	# 이때 반지름의 길이 r은 원점에서 원 위의 점까지의 길이로 생각할 수 있고
	# 원점에서 좌표까지의 길이는 √(rcosθ)²+(rsinθ)² = r임
	# 그런데 이때 r²/2는 ( (rcosθ)²+(rsinθ)² ) / 2이고
	# 이게 바로 p1이다. 
	# 이 p1이 평균이 1인 지수분포를 따르는 것이다. 
...
`-` 그렇다면 이것을 응용하여 정규분포를 생성할 수 있겠다. 
  - 정규분포를 따르는 건 rcosθ와 rsinθ이다.
  - 이때 r²/2이 평균이 1인 지수분포를 따른다는 것을 유념해두자
"""

# ╔═╡ b4c8ba40-9409-4ebb-b63c-cae56eebdf26
let 
	N= 10000
	R = .√(2*rand(Exponential(1),N)) # √연산자는 broadcasting할 때 앞에 dot을 붙여준다
	Θ = rand(N).*2π

	T = (R,Θ) -> (R*cos(Θ), R*sin(Θ)) # 이것들이 각각 독립으로 표준정규분포를 따를 것임
	XY = T.(R,Θ) # 반환되는 결과값이 comma로 이루어진 튜플로 반환될 것이다. 
	X = [XY[i][1] for i in 1:N] # Rcos(θ)
	Y = [XY[i][2] for i in 1:N] # Rsin(θ)
	# scatter(X,Y)
	
	p1=histogram(X)
	p2=histogram(Y)
	p3=randn(N) |> histogram # 실제 맞는지 비교
...
let 
	...
	U₂ = rand(N)
	X = @. √(-2log(1-U₁))*cos(2π*U₂)
	# 쩌~기 위에 inverse cdf할 때, 평균이 1인 지수분포를 따르는 함수 모양과 흡사함
	p1 = histogram(X) 
