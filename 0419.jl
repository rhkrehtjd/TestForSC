...
let
	...
	# 음 정리하자면 표준정규분포 제곱합과 지수분포 그리고 카이제곱분포는 관련이 있는데 표준정규분포 제곱합을 짝수개로 더하면 카이제곱분포와 연결시킬 수 있고, 물론 표준정규분포를 짝수개로 더해야만 카이제곱분포로 연결시킬 수 있는 건 아니다. 아무튼 이는 평균이 2인 지수분포 여러개를 더한 것으로도 연결시킬 수 있다. 
