...
`-` 포아송분포의 요약 
- X의의미
  - 발생횟수의 평균이 λ인 분포에서 실제 발생횟수를 X라고 한다. 
- X의범위
  - 발생안할수도 있으므로 X=0이 가능. 따라서 X=0,1,2,3,... 
  - 0 이상의 정수만 가능하다.
- parameter
  - λ = 평균적인 발생횟수
  - λ > 0
...
`-` 평균이 3인 포아송분포에서 100개의 샘플을 뽑는 방법 
  - 발생 횟수이기 때문에 0 혹은 자연수로 뽑힐 것
    - 평균이 소수이더라도 발생 횟수는 0 이상의 정수로 추출됨
"""
...
# ╔═╡ 94c47c49-4517-4526-b6db-227c668c1656
mean(rand(Poisson(3),10000))

# ╔═╡ 9a9ed77b-903e-4587-967c-518343a62e54
md"""
(방법2) 이항분포의 포아송근사를 이용 
- 이론
  - 이항분포에서   (1) $n\to \infty$   (2) $p\to 0$   (3) $np=\lambda$   이면 이것은 평균이 $\lambda$인 포아송분포로 근사함. 
  - 근사방법이므로 엄밀히 말하면 분포를 뽑는 방법이라고 말할 수 없음.
- 평균이 $\lambda$인 포아송분포는 $B(n,\frac{\lambda}{n})$를 이용하여 근사할 수 있다. 
  - 이때 $n$이 커질수록 더 정확해짐. 
"""
...
let 
	...
	# 그리고 이 60초 동안의 시간에 시행횟수는 600으로 설정했음
	# 따라서 한 번 시행할 때 0.01초가 소요됨.
    # Δt는 포아송 분포에서 사용하는 건 아니고 바로 아래 markdown 작성시 불러오려고 정의해준 것.
	
	poi_samplesize = 10000
	X = [(rand(n) .< p) |> sum for k in 1:poi_samplesize]
	# 이항분포를 카운팅하는 concept에서 포아송분포를 근사시킨 것.
	
	# (rand(n) .< p) -> 여기서 n을 600이라고 설정했는데 600개 하나하나가 베르누이 시행이 되는 것. 균등분포를 이용한 600번의 시행이 베르누이 분포를 이용한 600번의 시행이며, 이 bool형의 결과를 다 더해주면 그게 이항분포 한 번 시행의 결과로 근사시킬 수 있음
	#  rand(600) .< p |> sum  == B(600 , P) , 이때 p는 λ/n으로 이용할 것.
	# 이때 n이 커질수록, p는 0으로 갈수록 np=λ일 때, 포아송분포로 근사할 수 있다. 
	# 그리고 (rand(n) .< p) -> 이건 나열된 bool의 형태이기 때문에 다 더하면
	# scalar가 나올 것이고 이 스칼라는 이항분포 한 번 시행한 것이 됨.
	# 즉 sum까지하면 이항분포이며
	# 이걸 poi_samplesize만큼 수행했을 때 포아송분포로 근사할 수 있다.
	...
	- 개념을 잡아보자
	  ...
	  - 이건 사실 1초에 성공확률이 0.05인 1번의 베르누이 시행을 1분동안 총 60회 반복한 것으로 이해할 수 있음.
	  - 아니야, 이건 사실 $(Δt) 초에 성공확률이 $p 인 베르누이 시행을 1번 시행하여 1분동안 총 $n 회 반복한 것으로 이해할 수 있음.
	  - 아니야, 이건 사실.... (무한반복)
	    - 결론 : 매우 작은 시간에 엄청 작은 확률의 베르누이 시행이 독립적으로 엄청 많이 반복되는 뉘앙스
	$(_p)
	- 왼쪽 : 유니폼 -> 베르누이 -> 이항분포 -> 포아송
	- 오른쪽 : 포아송
	"""
end 
...
- 의미
  - 1분동안 맥도날드 매장에 들어오는 남자의 수는 평균이 5인 포아송 분포를 따름 
  - 1분동안 맥도날드 매장에 들어오는 여자의 수는 평균이 4.5인 포아송분포를 따름 
  - 남자와 여자가 매장에 오는 사건은 독립
    - 1분동안 맥도날드 매장에 오는 사람은 평균이 9.5인 포아송 분포를 따른다
"""
...
md"""
- 왜 평균의 추정치와 분산의 추정치가 달라야하나? 
  - 평균 추정치들과 분산 추정치들을 모아서 히스토그램을 그려보자
"""
...
let 
	n = 1000
	λ = 5 
	p1=[mean(rand(Poisson(λ),n)) for k in 1:10000] |> histogram
	p2=[var(rand(Poisson(λ),n)) for k in 1:10000] |> histogram
	p3=[(mean(rand(Poisson(λ),n))+var(rand(Poisson(λ),n))) /2  for k in 1:10000] |> histogram 
	# 그래도 평균추정이 제일 정확해보임
	# 섞어서 p3 만들어봤자 p2가 정확도가 떨어지는 값이라서 p3도 p1에 비해 좋지 않다.
	# 즉 p2가 제일 평균 λ에 근사한다.
...
md"""
`-` 지수분포의 요약 
- X의의미: 단위시간동안 평균 λ번 발생하는 사건이 있을때 첫 번째 이벤트가 발생할때 까지 걸리는 시간! 시간! 시간! 시간! 시간!, 따라서 이때의 평균은 $\frac{1}{\lambda}$겠지?
  - 모수가 λ인 푸아송분포는 어떤 사건이 단위시간 동안 평균 λ번 발생하는 것을 묘사하는 분포
  - 연속 확률 분포의 일종으로서, 사건이 서로 독립적일 때 일정 시간동안 발생하는 사건의 횟수가 푸아송 분포를 따른다면 next 사건이 일어날 때까지 대기시간은 지수분포를 따른다. 
- X의범위: 시간은 양수이므로 X ≥ 0
- parameter
  - (1) λ = 단위시간에 평균적으로 발생하는 사건의 수 
  - (2) 1/λ = 한번의 이벤트가 발생할때까지 평균적으로 걸리는 시간; 이때, λ>0 
- pdf
...
`-` 평균이 10인 지수분포에서 100개의 샘플을 뽑는 방법 
  - 단위시간동안 평균 발생횟수가 λ인 분포(포아송 분포)에서 첫번째 이벤트까지 걸리는 시간에 대한 분포
    - 따라서 평균이 $\frac{1}{\lambda}$인 지수분포를 따를 것이다.
"""
...
- 1명 방문하는데에는 평균적으로 시간이 10이 걸린다고 볼 수 있음. 
  - 지수 분포
- 따라서 언뜻생각하면 포아송과 지수 분포는 역의 관계라서 포아송 분포를 만들고 각 값들의 sample들에 대해서 역수를 취해주면 지수분포를 쉽게 만들 수 있을 것 같다.(사실 안 됨)
    - 왜 안 될까?
    - 만약 지수분포에서 분포값이 1.5가 나왔다면 푸아송분포는 그의 역수인 2/3가 나와야 함
    - 그런데 푸아송분포는 단위시간당 사건의 발생횟수가 λ인 분포이기 때문에 0혹은 양의 정수만 가능함
      - 푸아송 분포값들을 평균내면 소수가 나올 순 있겠지만(평균 추정치),
      - 푸아송 분포값 각각은 사건의 발생횟수이기 때문에 소수가 나올 수 없다.
      - 따라서 2/3이 푸아송분포의 값이 될 순 없음
- 즉, 각각의 값들을 역수 취해서 지수 분포 <-> 포아송 분포는 어렵다
- 그렇다면??
"""
...
- 각각의 sample들에 대해 역수를 취해주려고 했는데 0이 나온다....?
  - 즉 이 값들을 역수 취해서 지수분포를 만들 수 없음을 의미
  - 그렇다면 어떻게 지수분포로 근사시킬 수 있을까?
- 아이디어
  - 극한의 베르누이에서 시작해서 포아송을 만들 때, 몇 번 성공했는지 관심을 가지고 카운팅 했음
    - 균등분포 -> 베르누이분포 -> 이항분포 -> 포아송분포
  - 조금 응용해서 첫 성공까지 몇번의 시도를 해야하는지 카운팅을 한다고 생각하면 시간 계산(지수 분포)이 가능할것 같다.
  - 첫 성공까지 몇번의 시도?
      - 지수분포는 next사건이 발생할 때까지 소요되는 시간을 의미
      - 여기에 Δt를 곱해주면 지수분포의 값이 될 수 있을 것임
- 결국 포아송프로세스를 이용하여 지수분포로 가야함
"""
...
let 
	i=0 # 이렇게 i가 미리 지정돼있어야 while문이 일단 돌아갈 것임
	while i <= 5
		i=i+1
	end # while문 충족될 때까진 이 밑으로 안 내려감
	i
end
...
md"""
- 첫 성공(XXXXXXXXXXXO(첫성공))까지 몇번의 시도를 해야하는지 카운팅을 한다고 생각하면 시간계산이 가능할 것 같다
  - 그래서 아래 셀에서 histogram 그릴 때 델타t를 곱해주는 것!
"""

# ╔═╡ e5ac4d16-20ed-4c35-832f-b6fd5cfad2dd
function try_until_you_succeed(p) # 첫 성공까지의 횟수를 counting하는 함수 
	n_of_try=0 
	u=0
	while u < (1-p) 
		# p=0이면 무한반복 
		# u가 0.9보다 커야, 즉, 크기가 0.1안으로 들어와야 성공이라고 할 수 있음
		u = rand() # 균등분포 -> 베르누이 분포
		n_of_try = n_of_try + 1 
	end
	return n_of_try 
...
let
	p1 = [try_until_you_succeed(0.1) for k in 1:10000] |> histogram
	p2 = rand(Exponential(10), 10000) |> histogram
	# 지수분포의 평균이 10이라는 것
	# next 사건이 발생하려면 10 기다려야 한다는 거
	# 즉 푸아송분포에선 단위시간당 사건이 1/10 발생한다는 것
	# 즉 푸아송분포의 평균이 1/10이라는 것
	
	
	plot(p1,p2,layout=(1,2))
end

# ╔═╡ e8247017-0382-4d48-a87b-1a96cd145fbe
mean([try_until_you_succeed(0.1) for k in 1:10000])
# 기하분포임
# 기하분포 ? 동일한 베르누이 분포를 따르는 시행의 독립적인 반복에서 처음으로 성공하기까지의 시도횟수를 확률변수로 가지는 분포이다. 

# ╔═╡ a27efaae-2793-4d8a-8dd6-4900caa80a4a
let 
	exp_samplesize = 10000
	λ=0.1 # p2에서 모수가 10인 이상 이게 0.1이 아닐 순 없다
	n=60000
	p=λ/n
	Δt = (1/n)
	# 단위시간으로 생각해야 하니까 
