...
begin
	...
	N=100 
	r=2.52 # 95% 구간 추정할 수 있는 반지름
	X=randn(N).+ 1.2 # 표준정규분포
	Y=randn(N).+ 3.4 # 표준정규분포
	θ=0:0.01:2π
	
	_index = [(X[i]-1.2)^2 + (Y[i]-3.4)^2 ≤ r^2 for i in 1:N] 
	# Target 점과 사격수가 쏜 점 간의 거리
	# bool형으로 반환된다.
	
	scatter(X,Y,xlim=(-12,12),ylim=(-12,12),color="white",alpha=0.2)
	scatter!([1.2],[3.4],color="red") # Target, list로 넣는다
	plot!( 1.2 .+ r.*cos.(θ) , 3.4 .+ r.*sin.(θ) , color="red") # 극좌표로 표현된 직교좌표
...
- 그런데 문제는 3발이나 관측했고, 그 3발의 평균을 중심으로 원을 만들었기 때문에 좀 더 정확하게 추정이 가능할 것. 따라서 1탄때 사용했던 95% 추정을 위해 상정한 반지름(2.52)을 재차 사용하면 신뢰도는 95%보다 높을 것임. 그러니 95% 구간 추정하려면 반지름은 줄여야함
  - 1탄에 사용하였던 2.52라는 반지름은 95% 구간 추정하기에 큰 값임 
...
r2 = @bind r2 Slider(0.1:0.01:2.52,show_value=true)
# 일단 2.52보다 작아야하니까
...
begin
	...
	if side == "앞면"
		...
		scatter!([Xbar],[Ybar],color="pink",markershape=:cross) # 세발 관측치의 평균
		plot!(  Xbar .+ r2.*cos.(θ) , Ybar .+ r2.*sin.(θ), color="red" )
	else
...
# BOOL형으로 반환될테니 mean처리하면 확률값 반환과 동일하다
...
# ╔═╡ ddfb2ad5-4883-4c54-b6fd-79bca4310000
quantile([1,5,1,2,4,5,1],0.5)
# 중위수 뽑음
#  1 , 1 , 1 , `2` , 4 , 5 , 5 
# 평균 이런 거 아니고 그냥 가운데 숫자
...
- 중심이 (0,0) 이라고 가정
  - 어차피 해당되는 구간추정의 반지름만 찾으면 되기 때문에 중심은 임의의 점이어도 상관없음
"""
...
# ╔═╡ f870f509-e9c0-47c4-b6ac-7ad137e2f857
quantile([ √(mean(randn(3))^2 + mean(randn(3))^2) for i in 1:10000000 ], 0.95)

# ╔═╡ aa3be5ee-fc86-483d-a9d0-e87a402492b4
quantile( [√( (1.2 - mean(1.2 .+ randn(3)))^2 + (3.4 - mean(3.4 .+randn(3)))^2 ) for i in 1:10000000] ,0.95 )
...
# 평균이 0이라고 해놓고 니즈에 따라 수정하면 됨
# 일단 모평균은 μ로서 아직 미정임. 그냥 0으로 지정해두자.
